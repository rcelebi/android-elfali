...
import org.tensorflow.demo.YorumView;
...
/**
 * Class that takpublic TensorFlowImageListener(Activity ) {
  }es in preview frames and converts the image to Bitmaps to process with Tensorflow.
 */
class TensorFlowImageListener implements OnImageAvailableListener {
  ...
  public void onImageAvailable(final ImageReader reader) {
    ...
    final String recordURL =  "image_"+System.currentTimeMillis()+".png";

    // For examining the actual TF input.
    if (SAVE_PREVIEW_BITMAP) {
      ImageUtils.saveBitmap(croppedBitmap,recordURL);
    }
        ...
        new Runnable() {
          ...
          public void run() {
            ...
            if(detected) {
              ...
              StartUp.dbhelper.insertRecord(recordURL, result1, result2, result3,
                      result4);
              Intent myIntent = new Intent(activity, PalmView.class);
              myIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
              myIntent.putExtra(Properties.INTENT_EXTRA_PALM_URL, recordURL );
              myIntent.putExtra(Properties.INTENT_EXTRA_YORUM_RESULTS, new int[]{result1,result2,result3,result4});
          ...
          /*    Intent myIntent = new Intent(activity,
                      YorumView.class);
              myIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
              myIntent.putExtra(INTENT_EXTRA_PALM_URL,
                      HAND_PALM_IMAGE_EXT
                              + FILE_EXT);
              myIntent.putExtra(INTENT_EXTRA_YORUM_RESULTS, "ABC");
